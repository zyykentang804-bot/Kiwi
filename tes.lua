-- ============================================================================
-- SILENT HUB v1 - FULL BYPASS EMULATOR (ORIGINAL - FULL FEATURES)
-- ============================================================================
...
if not bypassActive then
    print("⚠️ Bypass emulator tidak aktif (executor tidak support / game tidak cocok)")
end
...
if not libLoaded then
    ...
    st.Size = UDim2.new(1,-20,0,60)
    st.Position = UDim2.new(0,10,0,60)
    st.BackgroundTransparency = 1
    st.Text = bypassActive and "✅ Bypass Emulator: AKTIF" or "⚠️ Bypass Emulator: TIDAK AKTIF\n(Library UI gagal dimuat)"
    st.TextColor3 = bypassActive and Color3.fromRGB(100,255,100) or Color3.fromRGB(255,200,100)
    st.TextWrapped = true
    st.Parent = f
    local c = Instance.new("TextButton")
    c.Size = UDim2.new(0,100,0,30)
    c.Position = UDim2.new(0.5,-50,1,-45)
    c.Text = "Tutup"
...
do
    ...
    do
        ...
        local aimToggle = AimSection:Toggle({ Name = "Enable Aimbot", Flag = "Aimbot_Enable", Default = false,
                        ...
                        if char then
                            ...
                            if part and hum and hum.Health > 0 then
                                ...
                                if onScreen then
                                    ...
                                    if dist < closestDist then
                                        closestDist = dist; closestPart = part; closestTarget = plr
                                    end
                                end
                            end
                        end
                ...
                pcall(function()
                    ...
                    for _, v in pairs(getgc(true)) do
                        if type(v)=="function" then
                            ...
                            if info and info.name == "CastBlacklist" then CastBL = v; break end
                        end
                    end
                    if CastBL then
...
do
    ...
    if not Drawing then
        ...
        local espEnabled = false; local espMaxDist = 600; local espColor = Color3.fromRGB(0,255,0); local fillBox = false; local showSkeleton = false; local showInventory = false; local espCache = {}
...
Library:Notification("Silent Hub v1 Loaded", "Bypass: " .. (bypassActive and "AKTIF" or "TIDAK AKTIF"), 5)
